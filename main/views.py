...
from main.models import ProductModel, LogOwnerModel, LogItemModel, StoreModel, MenuItemModel, CartModel, CartItemModel
from main.serializers import UserSerializer, GroupSerializer, ProductModelSerializer, ProductModelListSerializer, \
    ...
    CssTriangleErrorSerializer, StoreCreateSerializer, StoreResponseSerializer, StoreUpdateSerializer, \
    StorePublicSerializer, MenuItemCreateSerializer, MenuItemResponseSerializer, CartResponseSerializer, \
...
def widget_embed_code_generator(request):
    ...
    youtube_text = request.data.get('youtube_text', '')
    youtube_href = request.data.get('youtube_href', '')
    tiktok_text = request.data.get('tiktok_text', '')
    tiktok_href = request.data.get('tiktok_href', '')
    ...
    social_configs = {
        ...
        'youtube': {'buttonColor': '#FF0000', 'hoverColor': '#d10404'},
        'tiktok': {'buttonColor': '#000000', 'hoverColor': '#404040'}
    }
    ...
    social_data = [
        ...
        ('youtube', youtube_text, youtube_href),
        ('tiktok', tiktok_text, tiktok_href)
    ]
...
def store_create(request):
    """
    Create a new store (торговая точка).
...
def menu_item_create(request, write_uuid):
    """
    Create a new menu item (товар) for a store.
...
def cart_create(request, read_uuid):
    """
    Create a new cart (корзина) for a store.
